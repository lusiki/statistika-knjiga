...
<!-- =====================================================================
  ...
  1. Nastavni skup tjedna 13 (social_engagement.csv) knjizi nije dostupan.
     ...
     gdje su i nagib po godini dobi i učinci izvora ugrađeni u generator.
     Zbog toga se procjena može usporediti s vrijednošću koju procjenjuje,
     što nijedan stvarni skup podataka ne dopušta.
...
# dolazi iz R/podaci-nastavni.R, pa su vrijednosti koje modeli procjenjuju
# unaprijed poznate i svaka se procjena može usporediti s onim što procjenjuje.

# Vrijednosti ugrađene u generator populacije, doslovno iz R/podaci-nastavni.R.
s16_istina_dob <- 0.028
s16_istina_izvor <- c(
...
# Usporedba procijenjenih i stvarnih učinaka izvora.
s16_oporavak <- tibble(
  izvor = names(s16_istina_izvor),
  stvarno = unname(s16_istina_izvor),
  procjena = unname(coef(m16_dva)[paste0("izvor_vijesti", names(s16_istina_izvor))])
)
...
# Varijabla koja je posljedica ishoda pomaže predviđanju, a ne objašnjenju.
m16_posljedica <- lm(
...
s16 <- list(
  ...
  t_ttest = unname(abs(s16_t_test$statistic)),
...
```{r}
#| label: fig-pravac
#| fig-cap: "Prosječno povjerenje po godini dobi u simuliranoj populaciji, s pravcem najmanjih kvadrata procijenjenim na svih pedeset tisuća pojedinačnih odgovora."
...
```{r}
#| label: tbl-unutar
#| tbl-cap: "Nagib uz dob procijenjen zasebno među korisnicima svakog izvora vijesti, uz zbirni nagib i vrijednost ugrađenu u generator populacije. Izrada autora."
...
s16_unutar |>
  ...
  bind_rows(tibble(
    `Izvor vijesti` = c("Svi zajedno", "Ugrađeno u populaciju"),
    `Broj ljudi` = c(hr_broj(nrow(populacija_medija), 0), "—"),
    `Nagib uz dob` = c(
      hr_broj(s16$nagib_jedan, 4), hr_broj(s16$istina_dob, 3)
    )
  )) |>
...
```{r}
#| label: tbl-oporavak
#| tbl-cap: "Procijenjeni učinci izvora vijesti uz jednaku dob, uz vrijednosti ugrađene u generator populacije. Portal je referentna skupina. Izrada autora."

s16_oporavak |>
  transmute(
    `Izvor vijesti` = izvor,
    `Ugrađeno u populaciju` = hr_broj(stvarno, 2),
    `Procjena modela` = hr_broj(procjena, 2)
  ) |>
  knitr::kable(align = c("l", "r", "r"))
```
...
cbind(nagib = c(coef(zbirni)[["dob"]], coef(prilagodeni)[["dob"]]), usporedba)
```
...
#| tbl-cap: "Nagib uz dob u dvama modelima, s intervalom pouzdanosti, uz vrijednost ugrađenu u generator populacije. Izrada autora."

tibble(
  Model = c("Samo dob", "Dob i izvor vijesti"),
  Nagib = hr_broj(c(s16$nagib_jedan, s16$nagib_dva), 4),
  `Interval` = c(
    paste0(
      hr_broj(confint(m16_jedan)["dob", 1], 4), " do ",
      hr_broj(confint(m16_jedan)["dob", 2], 4)
    ),
    paste0(
      hr_broj(confint(m16_dva)["dob", 1], 4), " do ",
      hr_broj(confint(m16_dva)["dob", 2], 4)
    )
  ),
  `Ugrađeno` = hr_broj(rep(s16$istina_dob, 2), 3)
) |>

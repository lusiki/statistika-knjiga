...
U simulatoru zadana vjerojatnost modela ostaje nepromijenjena, a mijenja se samo
duljina niza. Vidljivo postaje ono zbog čega se kratki nizovi mogu pogrešno
...
Razlog je u načinu na koji se iz konačnih nizova bira i prosječuje. Kad iz
jednog niza izdvojimo mjesta iza tri pogotka, niz s malo pogodaka rijetko uopće
nudi takvo mjesto. U nizu koji ga nudi ta su tri pogotka već dio uvjeta, pa su
preostali ishodi koji mogu stajati na promatranome mjestu u prosjeku nešto
siromašniji pogocima od cijeloga niza. To nije memorija procesa, nego
kombinatoričko ograničenje uvjetnog odabira u konačnom nizu. Prosjek takvih
udjela zato leži ispod stvarne vjerojatnosti, i to više što je niz kraći i uvjet
dulji. Uz uvjet od samo jednog pogotka u nizu od stotinu pokušaja odstupanje
pada na `r hr_broj(s7$nakon_jednog_100, 3)` i jedva se primjećuje.
...
::: {.callout-model}
Pod zadanim modelom asistent može izračunati vjerojatnost i izvesti simulaciju.
Slaganje tih putova provjerava samo unutarnju dosljednost računa, a ne model
...
> pokaži da se dobiveni broj nalazi između nule i jedinice.

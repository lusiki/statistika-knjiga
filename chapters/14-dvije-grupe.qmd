...
s14_welch <- t.test(povjerenje_medijima ~ izvor, data = dvije_skupine)
s14_student <- t.test(
  povjerenje_medijima ~ izvor,
  data = dvije_skupine,
  var.equal = TRUE
)
s14_model <- lm(povjerenje_medijima ~ izvor, data = dvije_skupine)
s14_granice <- confint(s14_model)["izvorTV", ]
...
s14 <- list(
  ...
  donja = unname(s14_granice[[1]]),
  gornja = unname(s14_granice[[2]]),
  d = s14_d,
  df_welch = unname(s14_welch$parameter),
  df_student = unname(s14_student$parameter),
  var1 = var(s14_po_skupini[[1]]),
  var2 = var(s14_po_skupini[[2]]),

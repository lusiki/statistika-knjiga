...
<!-- =====================================================================
  ...
  4. Konstruirani primjer dvaju portala preuzet je iz tjedna 1, odjeljak 3.1.
     Brojke su izmišljene i u prozi su tako i označene.
...
# Konstruirani primjer za odjeljak o zbirnoj slici. Brojke su izmišljene i
# služe samo aritmetici, pa se u prozi izričito označavaju kao konstrukcija.
s1_portali <- tribble(
...
# Raspored prijava po odjelima. Ovo su stvarni podaci iz UCBAdmissions i nose
# težinu argumenta u razrađenom primjeru, pa se računaju, a ne prepisuju.
ucb_prolaznost <- ucb |>
  group_by(Dept) |>
  summarise(Prolaznost = sum(Freq[Ishod == "Primljeni"]) / sum(Freq), .groups = "drop") |>
  arrange(desc(Prolaznost))

.udio_prijava <- function(spol, odjeli) {
  100 * sum(ucb_odjeli$Prijave[ucb_odjeli$Spol == spol & ucb_odjeli$Dept %in% odjeli]) /
    sum(ucb_odjeli$Prijave[ucb_odjeli$Spol == spol])
}

s1$laksi <- ucb_prolaznost$Dept[1:2]
s1$tezi <- rev(ucb_prolaznost$Dept)[1:2]
s1$laksi_m <- .udio_prijava("Muškarci", s1$laksi)
s1$laksi_z <- .udio_prijava("Žene", s1$laksi)
s1$tezi_m <- .udio_prijava("Muškarci", s1$tezi)
s1$tezi_z <- .udio_prijava("Žene", s1$tezi)
s1$prolaznost_max <- max(ucb_prolaznost$Prolaznost)
s1$prolaznost_min <- min(ucb_prolaznost$Prolaznost)
```
...
[@bickel1975]. Doseg tvrdnje nikada nije širi od skupa koji je ušao u izračun,
...
ucb_ukupno |>
  transmute(
    Spol,
    Prijave = hr_broj(Prijave, 0),
    Primljeni = hr_broj(Primljeni, 0),
    `Stopa prijma` = paste0(
      hr_broj(100 * Stopa, 1),
      "\u202f%"
    )
...
p_ucb_odjeli <- ucb_odjeli |>
  ...
  scale_y_continuous(
    labels = function(x) {
      paste0(hr_broj(100 * x, 0), " %")
    }
  ) +
  scale_linetype_manual(
    values = c("Muškarci" = "solid", "Žene" = "22")
  ) +
  scale_shape_manual(
    values = c("Muškarci" = 16, "Žene" = 1)
  ) +
  labs(
    x = NULL,
    y = "Stopa prijma",
...
p_ucb_odjeli + scale_color_knjiga()
```
:::
...
#| fig-alt: "Šest odjela s po dvije stope prijma. U četiri odjela stopa za žene jednaka je ili viša, iako je zbirna stopa za žene niža."
iscrtaj_figuru(
  p_ucb_odjeli + scale_color_knjiga(varijanta = "tamna"),
  "tamna"
)
```
:::
...
u najselektivnijem [@bickel1975]. Na dva najpristupačnija odjela otpada
...
[@bickel1975]. Dvije skupine zapravo nisu prolazile kroz isti postupak

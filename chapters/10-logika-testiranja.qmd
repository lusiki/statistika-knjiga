...
Jedina je greška posljednja rečenica, u kojoj p-vrijednost postaje vjerojatnost
hipoteze. Kod ispravno provodi obostranu usporedbu i korekciju za konačan broj
nasumičnih premještanja, a obrazloženje ispravno navodi nulti model,
zamjenjivost i granicu uzročne tvrdnje. Izračun se odnosi na rezultate koje
nulti model proizvodi, dakle uvjetovan je njegovim vrijedenjem, dok pogrešna
rečenica tvrdi kolika je vjerojatnost samog modela nakon podataka. Popravak je
napisati da bi nulti model proizveo razliku ovoliku ili veću uz korigiranu
p-vrijednost `r hr_broj(s10$p, 3)`, i uz to navesti procjenu razlike s
intervalom.
:::
:::
...
::: {.content-visible when-profile="kolegij"}
Dvije standardne devijacije daju približne granice
`r hr_broj(-2 * s10$sd_nulte, 2)` i `r hr_broj(2 * s10$sd_nulte, 2)` boda.
One su gruba normalna aproksimacija, dok je vrijednost
`r hr_broj(s10$granica, 2)` dobivena iz stvarne permutacijske raspodjele.
Testna je statistika sirova razlika sredina, a postupak je obostrani
permutacijski test.
Dobrovoljni uzorak s jednoga portala ne podupire generalizaciju na sve odrasle
osobe u Hrvatskoj. Test može uredno opisati slučajnost pod svojim nultim
modelom, ali ne popravlja selekcijski mehanizam. Kad je stvarna razlika u HTML
widgetu nula, udio ispod praga trebao bi biti blizu zadanom pragu; zbog konačnog
broja ponavljanja neće mu uvijek biti jednak. Prvi redak tiskane tablice daje
istu vrstu provjere pod poznatim nultim modelom i pokazuje
`r hr_broj(s10$stopa_nulta, 1)` %.
:::

...
<!-- =====================================================================
  STATUS:  djelomični nacrt — strukturiran vođeni projekt i protokol rada s asistentom
...
tibble(
  Mjera = c(
    "Broj opažanja",
    "Prosjek izloženosti",
    "Standardna devijacija izloženosti",
    "Prosjek znanja",
    "Standardna devijacija znanja"
  ),
  Vrijednost = c(
    format(opis_projekta$opažanja, big.mark = " ", trim = TRUE),
    formatC(
      opis_projekta$prosjek_izlozenosti,
      format = "f",
      digits = 1,
      decimal.mark = ","
    ),
    formatC(
      opis_projekta$sd_izlozenosti,
      format = "f",
      digits = 1,
      decimal.mark = ","
    ),
    formatC(
      opis_projekta$prosjek_znanja,
      format = "f",
      digits = 1,
      decimal.mark = ","
    ),
    formatC(
      opis_projekta$sd_znanja,
      format = "f",
      digits = 1,
      decimal.mark = ","
    )
...
```{r}
#| echo: true
p_projekt <- ggplot(sim_projekt, aes(izlozenost, znanje)) +
  ...
  labs(
    x = "Simulirana izloženost",
    y = "Simulirani rezultat znanja"
  ) +
  theme(
...
::: {#fig-projekt-odnos .responsive-static-figure}
::: {.figure-theme-light .figure-viewport-wide}
```{r}
#| echo: false
...
::: {.figure-theme-dark .figure-viewport-wide}
```{r}
#| echo: false
#| fig-alt: "Raspršeni dijagram pokazuje pozitivan odnos simulirane izloženosti i rezultata znanja."
#| fig-width: 5.8
#| fig-height: 4.8
#| fig-bg: transparent
iscrtaj_figuru(p_projekt, "tamna")
```
:::
:::

::: {.content-visible when-format="html"}
::: {.figure-theme-light .figure-viewport-mobile}
```{r}
#| echo: false
#| fig-alt: "Raspršeni dijagram pokazuje pozitivan odnos simulirane izloženosti i rezultata znanja."
#| fig-width: 4.2
...
::: {.figure-theme-dark .figure-viewport-mobile}
```{r}
#| echo: false
#| fig-alt: "Raspršeni dijagram pokazuje pozitivan odnos simulirane izloženosti i rezultata znanja."
#| fig-width: 4.2
...
bind_rows(
  ...
  pivot_longer(
    c(procjena, standardna_pogreska),
    names_to = "mjera",
  ...
  mutate(
    Mjera = recode(
      mjera,
      procjena = "Procjena",
      standardna_pogreska = "Standardna pogreška"
    ),
    Vrijednost = formatC(
      vrijednost,
      format = "f",
  ...
  select(Mjera, model, Vrijednost) |>
  pivot_wider(names_from = model, values_from = Vrijednost) |>

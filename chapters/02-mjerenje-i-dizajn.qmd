...
<!-- =====================================================================
  ...
  1. Cijelo tijelo poglavlja preneseno je iz tjedna 1, odjeljci 4 do 12.
     Redoslijed izlaganja je zadržan, proza je izvorna.
  2. Stevens 1946 dobio je unos stevens1946, provjeren preko Crossrefa. Plan ga
     je vodio kao dug rundi 1.
  3. Navarrov kandidat o „problemu treće varijable" uklonjen je u
     P1B-NAVARRO. Inačica 0.6 opisuje konfundiranje, ali ne podupire pripisani
     naziv ni tvrdnju; samostalno objašnjenje u poglavlju ostaje bez oslanjanja
     na taj izvor. Trag provjere čuva
     notes/reports/p1b-navarro-provenance-and-licence-audit-2026-08-03.md.
  4. Prag Cronbachova alfe iz predavanja NIJE prenesen jer za nj nema unosa u
     references.bib. Umjesto broja stoji tvrdnja da je granica konvencija o
     kojoj se raspravlja (H7).
  5. Okvir za komunikologiju proširen je na društvene znanosti u cjelini.
  6. Razrađeni primjer je nov. Zamjenjuje raniju tablicu od šest redaka i
     pokazuje okretanje niječne tvrdnje kao stvaran mjerni postupak. Podaci su
     izmišljeni i u prozi su tako označeni. Sav kod ostaje pogonski jer Dio I
     nema vidljivog koda.
...
razilaze, najprije treba utvrditi jesu li mjerili isto; tek se potom pita tko je
pogriješio.
...
Anketno je pitanje dio mjernog instrumenta. Formulacija određuje što ispitanik
razumije, ponuđeni odgovori
...
onoga kojemu je isto pitanje postavljeno prvo. Ta je mogućnost razlog da
rezultate anketa čitamo zajedno s upitnikom.
...
Zato najprije treba odvojiti jedinicu koja je opažena od jedinice o kojoj se
...
prosvjed. Nacrt pokazuje red odluka i ne daje empirijski nalaz. Jedinica
...
koliko je jedinica uklonjeno i zbog čega. Zaključak se tada odnosi na užu
skupinu koja je preživjela ta pravila, umjesto na sve priloge iz početnog
okvira.
...
podjelu na četiri razine mjerenja [@stevens1946]. Ona opisuje jesu li vrijednosti
...
dosegnuti. Umjesto potrage za općenito najboljim dizajnom, pitamo koji može
nositi tvrdnju koja se postavlja.
...
smjer djelovanja za uže pitanje. Kvalitativno istraživanje
...
Kasniji će računi obuhvatiti dio uzorkovne i modelne nesigurnosti. Dijelovi
ovoga popisa ostaju izvan njihova dosega. Zato uz svaki budući interval ili
...
viewof w02_par = Inputs.form({
  ...
  pomak: Inputs.range(
    [0, 5],
    {
      value: 4,
      step: 0.25,
      label: "Dodatni pomak ishoda pri jednakoj izloženosti"
    }
  )
})
```
...
1. U lijevom panelu odredite smjer zbirne veze izloženosti i ishoda.
2. U desnom panelu zasebno odredite smjer veze unutar svake razine treće
   varijable.
3. Usporedite dva odgovora i objasnite zašto zbirni pravac ne opisuje samo
   odnos izloženosti i ishoda.
:::
...
odluke, nakon povlačenja ili preusmjeravanja dijela prijava [@bickel1975].
...
::: {.callout-model}
Od asistenta najprije tražimo nacrt varijabli i upozorenje na moguće
konfundere. Njegov popis nije dokaz da su mjere valjane. Treba provjeriti
...
::: {#ex-02-callout-greska-01 .callout-greska}
...
# Ručno zadani odgovori 12 ispitanika. Skup je izmišljen i služi prikazu
# postupka, pa se u prozi nikada ne opisuje kao nalaz o populaciji.
s2_odgovori <- tribble(
...
s2_stavke <- c("t1", "t2", "t3", "t4")

# Korelacija svake tvrdnje sa zbrojem preostalih. Ovo je dijagnostika, a ne
# konačna mjera pouzdanosti; vidi prozu ispod tablice.
s2_stavka_ukupno <- function(podaci, stavke) {
  vapply(
    stavke,
    function(s) {
      ostatak <- rowSums(podaci[setdiff(stavke, s)])
      stats::cor(podaci[[s]], ostatak)
    },
    numeric(1)
  )
}

s2_prije <- s2_stavka_ukupno(s2_odgovori, s2_stavke)

s2_okrenuto <- s2_odgovori |>
  mutate(t4 = 6 - t4)

s2_poslije <- s2_stavka_ukupno(s2_okrenuto, s2_stavke)

s2_rezultati <- s2_okrenuto |>
  mutate(povjerenje = rowMeans(pick(all_of(s2_stavke))))

s2_naivno <- s2_odgovori |>
  mutate(povjerenje = rowMeans(pick(all_of(s2_stavke))))

s2 <- list(
  raspon_ispravno = diff(range(s2_rezultati$povjerenje)),
  raspon_naivno = diff(range(s2_naivno$povjerenje)),
  t4_prije = s2_prije[["t4"]],
  t4_poslije = s2_poslije[["t4"]]
)
```
...
:::: {.table-keep-words}
```{r}
#| label: tbl-mjerenje-stavke
#| tbl-cap: "Povezanost svake tvrdnje s ostatkom instrumenta, prije i nakon okretanja niječne tvrdnje. Izrada autora."
...
tibble(
  ...
  `Prije okretanja` = hr_broj(s2_prije, 2),
  `Nakon okretanja` = hr_broj(s2_poslije, 2)
) |>
...
#| tbl-cap: "Odgovori nakon okretanja niječne tvrdnje i izvedena mjera povjerenja. Izrada autora."

s2_rezultati |>
  transmute(
    Ispitanik = ispitanik,
    `Odgovori na tvrdnje 1 do 4` = paste(t1, t2, t3, t4, sep = " · "),
    Povjerenje = hr_broj(povjerenje, 2)
  ) |>
  head(6) |>
...
### Konceptualni {#ex-02-konceptualni-01 .zadaci-razina}
...
### Računski {#ex-02-racunski-01 .zadaci-razina}
...
### Kritički {#ex-02-kriticki-01 .zadaci-razina}
...
### Revizija modela {#ex-02-revizija-modela-01 .zadaci-razina}
